...
use app\models\PtOrderDetail;//P_ADD
use app\models\PrinceAccountLog;//P_ADD
use app\models\PrinceStoreAccount;//P_ADD
...
class OrderBehavior extends BaseBehavior
{
    ...
    public function beforeAction($e)
    {
	    ...
	    $this->princeToaccount($value['id']);//P_ADD
        }
	    ...
	    $this->princeMsToaccount($value['id']);//P_ADD
        }
	    ...
	    $this->princePtToaccount($value['id']);//P_ADD
        }
...
//        }
    //P_ADD start
    private function princeToaccount($order_id)
    {
		...
		$goods_list = OrderDetail::find()->alias('od')
            ->leftJoin(PrinceGoodsRelation::tableName() . ' pgr', "od.goods_id = pgr.goods_id ")
            ->leftJoin(Goods::tableName() . ' g', "pgr.cloud_goods_id = g.id ")
            ->select('od.num,g.cost_price')
            ->where(['od.order_id' => $order_id, 'od.is_delete' => 0, 'pgr.type' => $type])
            ->asArray()->all();
		$total_cost_price=0;
        foreach ($goods_list as $good) {
            $total_cost_price=$total_cost_price+$good['num']*$good['cost_price'];
        }
		$share_price=$order->first_price+$order->second_price+$order->third_price+$order->rebate;
		$price=$order->pay_price-$order->express_price-$total_cost_price-$share_price;

		$desc.=$order->order_no.'结算(实付款'.$order->pay_price.'-运费'.$order->express_price.'-成本价'.$total_cost_price.'-分销佣金'.$share_price.')';
		$this->putToaccount($price,$desc);
    }
    private function princeMsToaccount($order_id)
    {
        $list = MsOrder::find()->alias('mo')
            ->leftJoin(PrinceGoodsRelation::tableName() . ' pgr', "mo.goods_id = pgr.goods_id ")
            ->leftJoin(Goods::tableName() . ' g', "pgr.cloud_goods_id = g.id ")
            ->select('mo.order_no,mo.pay_price,mo.express_price,mo.num,mo.first_price,mo.second_price,mo.third_price,mo.rebate,g.cost_price,pgr.cloud_goods_id')
            ->where(['mo.id' => $order_id, 'mo.is_delete' => 0, 'pgr.type' => 1])
            ->asArray()->all();
		$order=$list[0];
		if($order['cloud_goods_id']){
			$total_cost_price=$order['num']*$order['cost_price'];
			$share_price=$order['first_price']+$order['second_price']+$order['third_price']+$order['rebate'];
			$price=$order['pay_price']-$order['express_price']-$total_cost_price-$share_price;
			$desc="秒杀订单".$order['order_no'].'结算(实付款'.$order['pay_price'].'-运费'.$order['express_price'].'-成本价'.$total_cost_price.'-分销佣金'.$share_price.')';
			$this->putToaccount($price,$desc);
		}else{
			$order = MsOrder::findOne($order_id);
			$desc="秒杀订单".$order['order_no'].'结算';
			$this->putToaccount(0,$desc);
		}
    }
    private function princePtToaccount($order_id)
    {
        $list = PtOrder::find()->alias('po')
            ->leftJoin(OrderShare::tableName() . ' os', "os.order_id = po.id ")
            ->leftJoin(PtOrderDetail::tableName() . ' pod', "pod.order_id = po.id ")
            ->leftJoin(PrinceGoodsRelation::tableName() . ' pgr', "pod.goods_id = pgr.goods_id ")
            ->leftJoin(Goods::tableName() . ' g', "pgr.cloud_goods_id = g.id ")
            ->select('po.pay_price,po.express_price,pod.num,os.first_price,os.second_price,os.third_price,os.rebate,g.cost_price,pgr.cloud_goods_id')
            ->where(['po.id' => $order_id, 'po.is_delete' => 0, 'pgr.type' => 2,'os.type' => 0])
            ->asArray()->all();
		$order=$list[0];
		if($order['cloud_goods_id']){
			$total_cost_price=$order['num']*$order['cost_price'];
			$share_price=$order['first_price']+$order['second_price']+$order['third_price']+$order['rebate'];
			$price=$order['pay_price']-$order['express_price']-$total_cost_price-$share_price;
			$desc="拼团订单".$order['order_no'].'结算(实付款'.$order['pay_price'].'-运费'.$order['express_price'].'-成本价'.$total_cost_price.'-分销佣金'.$share_price.')';
			$this->putToaccount($price,$desc);
		}else{
			$order = PtOrder::findOne($order_id);
			$desc="拼团订单".$order['order_no'].'结算';
			$this->putToaccount(0,$desc);
		}
    }
    private function putToaccount($price,$desc)
    {
        $sa = PrinceStoreAccount::findOne(['store_id' =>$this->store_id]);
        if (!$sa) {
			$sa = new PrinceStoreAccount;
			$sa->money =$price;
			$sa->store_id = $this->store_id;
        }else{
			$sa->money = $sa->money + $price;
		}
		$sa->save(false);
