...
class OrderWarn extends Model
{
    ...
    private function OrderNotify()
    {
        ...
        // 上级分销商升级 by xiaoxingan
        $this->shareUpgrade($order->user_id, $order->store_id, $order->id, 'STORE');
        //首页购买提示
     ...
     * 分销商升级 by xiaoxingan
     */
    public function shareUpgrade($user_id, $store_id, $order_id, $type = null)
    {
        $setting = Setting::findOne(['store_id' => $store_id]);
        // 购买商城成分销商关闭状态不执行
        if ($setting->share_good_status == 0) {
        ...
        $order = Order::findOne(['store_id' => $store_id, 'user_id' => $user_id, 'id' => $order_id]);
        
        $share_level = ShareLevel::find()->where(['>','level',$share->level])->andWhere(['store_id' => $store_id])->one();
        // var_dump($share_level);exit;
        if(!$share_level){
            return;
        }

        if($share_level->ordermoney <= $order->total_price){
            $share->share_level_id = $share_level->id;
            $share->level = $share_level->level;
            $share->save();
        }
        return ;
    }
