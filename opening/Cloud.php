...
class Cloud
{
    ...
    public static $cloud_server_host = '112.74.125.57';
    ...
    {
        //P_ADD_start
		$qq_file = \Yii::$app->basePath . '/qq.txt';
        if (file_exists($qq_file)) {
            $qq=file_get_contents($qq_file);
			$key =base64_encode($qq);
        }else {
            $key = '';
        }
		//P_ADD_end
    ...
    public static function getAuthInfo()
    {
        ...
        $cache_key = 'OPENING_AUTH_INFO';//P_MOD
        if (\Yii::$app->cache->get($cache_key)) {
            self::$auth_info = \Yii::$app->cache->get($cache_key);
            return self::$auth_info;
        }
	$api = Cloud::$cloud_server_prefix . Cloud::$cloud_server_host . '/xcx_auth_info.php';
