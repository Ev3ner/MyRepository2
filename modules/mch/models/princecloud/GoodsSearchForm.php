...
class GoodsSearchForm extends MchModel
{
    ...
    {
        $goods = $this->goods;
        $catList = Cat::find()->where(['store_id' => $this->cloud_store_id, 'is_delete' => 0, 'parent_id' => 0])->all();
        $postageRiles = PostageRules::find()->where(['store_id' => $this->cloud_store_id, 'is_delete' => 0])->all();
        $cardList = Card::find()->where(['store_id' => $this->cloud_store_id, 'is_delete' => 0])->asArray()->all();
        if ($goods->full_cut) {
    ...
    public function getList()
    {
        ...
        $status = $this->status;
        $query = Goods::find()->alias('g')->where(['g.store_id' => $this->cloud_store_id, 'g.is_delete' => 0, 'g.mch_id' => 0]);
        ...
        $query->andWhere(['or', ['gc.is_delete' => 0, 'gc.store_id' => $this->cloud_store_id], 'isnull(gc.id)']);

        $cat_query = clone $query;

        $query->select('g.id,g.name,g.price,g.cost_price,g.original_price,g.status,g.cover_pic,g.sort,g.attr,g.cat_id,g.virtual_sales,g.store_id,g.quick_purchase');
        if (trim($keyword)) {
