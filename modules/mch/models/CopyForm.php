...
class CopyForm extends MchModel
{
    ...
    public function copy()
    {
        ...
        if (strpos($this->url, '1688.com') || is_numeric($this->url)) {
			...
			$data = $this->p_1688_copy($itemid,$type);
            if ($data['code'] == 1) {
                return $data;
            }
            return [
        ...
        if (strpos($this->url, 'yangkeduo.com') ) {
            $id = $this->preg_substr('/(\?goods_id=|&goods_id=)/', '/&/', $this->url);
            $data = $this->p_pdd_copy($id[0]);
            if ($data['code'] == 1) {
                return $data;
            }
            return [
...
//            $id = $this->preg_substr('/&?id=/', '/&/', $arr[1]);
    ...
    //2_P_ADD_start
    public function p_tb_copy($id)
    {
        $api = "http://112.74.125.57/xcx_tb_copy.php";
        $curl = Cloud::apiGet($api, ['vid'=>$id,'host'=> \Yii::$app->request->hostName]);
    ...
    public function p_1688_copy($id,$type)
    {
    ...
    public function p_jd_copy($id)
    {
        $api = "http://112.74.125.57/xcx_jd_copy.php";
        $curl = Cloud::apiGet($api, ['vid'=>$id,'host'=> \Yii::$app->request->hostName]);
        $res = json_decode($curl->response, true);
        if ($res && isset($res['code'])) {
            if ($res['code'] != 0) {
    ...
    public function p_pdd_copy($id)
    {
        $api = "http://112.74.125.57/xcx_pdd_copy.php";
        $curl = Cloud::apiGet($api, ['vid'=>$id,'host'=> \Yii::$app->request->hostName]);
        $res = json_decode($curl->response, true);
        if ($res && isset($res['code'])) {
            if ($res['code'] != 0) {
    ...
    private function p_data($html = null)
    {
        $info = $html;
		if(isset($info['prince'])&& $info['prince']==1){
			//标题
			$title = $info['title'];
			//缩略图
			...
			//售价
			$goods_price_sale = $info['goods_price_sale'];
			//销量
			$sale_count =$info['sale_count'];
			//库存
			$quantity = $info['quantity'];
			//规格列表
			$attr_group_list = $info['attr_group_list'];
			//规格库存列表
			$checked_attr_list = $info['checked_attr_list'];
			//图文详情
...
//            }
//        }
        $api = "http://112.74.125.57/xcx_tb_copy.php";//P_MOD
        $curl = Cloud::apiGet($api, ['vid'=>$id,'host'=> \Yii::$app->request->hostName]);//P_MOD
        $res = json_decode($curl->response, true);//P_MOD
        if ($res && isset($res['code'])) {
            if ($res['code'] != 0) {
                return [
                    'code'=>1,
