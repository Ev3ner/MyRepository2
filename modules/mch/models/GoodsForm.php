    ...
    public function save()
    {
        if ($this->validate()) {
            ...
            if ($goods->save()) {
				...
				//P_ADD start
				if(get_plugin_type()==2 && $this->mall_id){
					$goods_relation=PrinceGoodsRelation::findOne(['goods_id' => $this->mall_id, 'type' => 0, 'store_id' => $this->store_id]);
					if($goods_relation){
						$kj_goods_relation=PrinceGoodsRelation::findOne(['goods_id' => $goods->id, 'type' => 3, 'store_id' => $this->store_id]);
						if(!$kj_goods_relation){
							$kj_goods_relation=new PrinceGoodsRelation();
						}
						$kj_goods_relation->store_id = $this->store_id;
						$kj_goods_relation->cloud_goods_id = $goods_relation->cloud_goods_id;
						$kj_goods_relation->goods_id = $goods->id;
						$kj_goods_relation->type = 3;
						$kj_goods_relation->save(false);
					}
				}
				//P_ADD end
