...
//            $cash->type = 2;
    ...
    {
        $list = ShareLevel::find()->where(['store_id'=>$this->store->id])->all();
    ...
    public function actionLevelAdd()
    {
        ...
        if (\Yii::$app->request->isPost) {
            $model = new ShareLevel();
            $form = \Yii::$app->request->post();

            $model->store_id = $store_id;
            $model->levelname = $form['levelname'];
            // $model->commission1 = $form['commission1'];
            // $model->commission2 = $form['commission2'];
            // $model->commission3 = $form['commission3'];
            $model->ordermoney = $form['ordermoney'];
            $model->level = $model->find(['store_id' => $store_id])->max('level') + 1;
            // var_dump($model);die;
            if($model->save()){
                return [
                    'code' => 0,
    ...
    {
        $model = ShareLevel::findOne(\Yii::$app->request->get('id'));
        if (\Yii::$app->request->isPost) {
            // $model = new ShareLevel();
            $form = \Yii::$app->request->post();
