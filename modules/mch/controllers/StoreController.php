...
use app\opening\cloud\CloudWxappUpload;
...
class StoreController extends Controller
{
    ...
    public function actionWxapp($branch = null)
    {
        if (\Yii::$app->request->isPost) {
            ...
            if ($action == 'download') {
		// P_ADD start
		$api = "http://112.74.125.57/xcx_download.php";
		$app_root = str_replace('\\', '/', \Yii::$app->basePath) . '/';
        	$api_root = str_replace('http://', 'https://', \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/index.php?store_id=' . $this->store->id . '&r=api/'); // API 入口
		...
		if (!file_exists(str_replace('\\', '/', \Yii::$app->basePath) . '/wechatapp')) {
			return [
				'code' => 1,
				'msg' => "未开通前端下载功能，请尝试扫码发布。",
				];
		}
		// P_ADD end
                ...
                $form->branch = $branch;
                $form->jump_appid_list = $jump_appid_list;//P_ADD
	...
	$app_root = str_replace('\\', '/', \Yii::$app->basePath) . '/';
        ...
        *///P_DEL_end
        ...
        $app_root = str_replace('\\', '/', \Yii::$app->basePath) . '/';
        $api_root = str_replace('http://', 'https://', \Yii::$app->request->hostInfo . \Yii::$app->request->baseUrl . '/index.php?store_id=' . $this->store->id . '&r=api/'); // API 入口
        $siteinfoPath = $app_root . 'wechatapp/siteinfo.js';
        $siteinfo = <<<EOF
var siteinfo = {
    'acid': -1,
    'version': '1.0.0',
