            ...
            <table class="table table-bordered bg-white">
                            ...
                            <?php if ($order_item['is_delete'] == 0): ?>
                                <span class="mb-2">
                                        <a class="btn btn-sm btn-info apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/miaosha/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 1]) ?>">同意取消</a>
                                    </span>
                                <span class="mb-2">
                                        <a class="btn btn-sm btn-danger refuse apply-status-btn"
                                           href="<?= $urlManager->createUrl(['mch/miaosha/order/apply-delete-status', 'id' => $order_item['id'], 'status' => 0]) ?>">拒绝取消</a>
                                    </span>
                            <?php endif; ?>
                        <?php endif; ?>
                    </td>
                ...
                <tr>
                    ...
                    <td class="order-tab-2">
                        <div class='titleColor mt-3'>
                            <span>总金额：<span style="color:blue;"><?= $order_item['total_price'] ?></span>元</span>
                            ...
                            <?php if ($order_item['user_coupon_id']): ?>
                                <div>优惠券：<?= $order_item['coupon_sub_price'] ?>元</div>
                            <?php endif; ?>
                            ...
                            <?php if ($order_item['integral'] && $order_item['integral']['forehead_integral']): ?>
                                <div>使用<?= $order_item['integral']['forehead_integral'] ?>积分抵扣：<span
                                            style="color:red;"><?= $order_item['integral']['forehead'] ?></span>元
                                </div>
                            <?php endif; ?>
                            <?php if ($order_item['discount'] && $order_item['discount'] != 10): ?>
                                <div>会员折扣：<span style="color:red;"><?= $order_item['discount'] ?></span>折</div>
                            <?php endif; ?>
                    ...
                    <td class="order-tab-3">
                        <div class="mt-3"><span style="color:blue;"><?= $order_item['pay_price'] ?></span>元</div>
                    </td>
                    <td class="order-tab-4">
                        <?php if ($order_item['pay_type'] == 2): ?>
                            <div>
                                支付方式：
                                <span class="badge badge-success mt-3">货到付款</span>
                            </div>
                        <?php elseif ($order_item['pay_type'] == 3): ?>
                            <div>
                                支付方式：
                                <span class="badge badge-success mt-3">余额支付</span>
                            </div>
                        <?php else: ?>
                            <div>
                                支付方式：
                    ...
                    <?php endif; ?>
                        <div class="mt-2">
                            <a class="btn btn-sm btn-info del" href="javascript:"
                               data-url="<?= $urlManager->createUrl(['mch/order/print-order', 'order_id' => $order_item['id'], 'order_type' => 1]) ?>"
                        ...
                        <?php if ($order_item['is_recycle'] == 1) : ?>
                            <div>
                                <a class="btn btn-sm btn-primary mt-2 del" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/miaosha/order/recycle', 'order_id' => $order_item['id'], 'is_recycle' => 0]) ?>"
                                   data-content="是否移出回收站">移出回收站</a>
                            </div>
                        <?php else : ?>
                            <div>
                                <a class="btn btn-sm btn-danger mt-2 del" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/miaosha/order/recycle', 'order_id' => $order_item['id'], 'is_recycle' => 1]) ?>"
                        ...
                        <?php if ($order_item['is_recycle'] == 1) : ?>
                            <div>
                                <a class="btn btn-sm btn-danger mt-2 del" href="javascript:"
                                   data-url="<?= $urlManager->createUrl(['mch/miaosha/order/delete', 'order_id' => $order_item['id']]) ?>"
                        ...
                        <?php if ($order_item['pay_type'] == 2 && $order_item['is_send'] == 1 && $order_item['is_confirm'] != 1): ?>
                            <div>
                                <a href="javascript:" class="btn btn-sm btn-primary mt-2 del"
                                   data-url="<?= $urlManager->createUrl(['mch/miaosha/order/confirm', 'order_id' => $order_item['id']]) ?>"
                ...
                <tr>
                    <td colspan="5">
                        <?php if ($order_item['is_offline'] == 0): ?>
                            <span class="mr-2"><span class="titleColor">收货人：</span><?= $order_item['name'] ?></span>
                            ...
                            <?php if ($order_item['is_send'] == 0) : ?>
                                <a class="btn btn-sm btn-primary edit-address"
                                   data-index="<?= $k ?>"
                                   data-order-type="miaosha" href="javascript:">修改地址</a>
                            <?php endif; ?>
                        <?php else: ?>
                        ...
                        <?php if ($order_item['content']): ?>
                            <div><span class="titleColor">备注：</span><?= $order_item['content'] ?></div>
                        <?php endif; ?>
                    </td>
                </tr>
            </table>
...
<script>
    ...
    $(document).on("click", ".send-confirm-btn", function () {
        ...
        console.log(error);
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/miaosha/order/send'])?>",
            type: "post",
...
<script>
    ...
    $(document).on('click', '.print', function () {
        ...
        $.ajax({
            url: "<?=$urlManager->createUrl(['mch/miaosha/order/print'])?>",
            type: 'get',
            dataType: 'json',
            data: {
                id: id,
...
<script>
    ...
    $(document).on('click', '.change-price', function () {
        ...
        var url = "<?=$urlManager->createUrl(['mch/miaosha/order/update-price'])?>"
        $.ajax({
            url: url,
            type: 'get',
            dataType: 'json',
