...
//            $form->scenario = "NONE_CONDITION";
    ...
    public function actionGetInfo()
    {
        ...
        // 获取分销商等级 by xiaoxingan
        $levelname = '';
        $share = Share::findOne(['user_id'=>\Yii::$app->user->identity->id,'store_id'=>$this->store_id]);
        if($share){
            $share_level = ShareLevel::findOne(['id'=>$share->share_level_id]);
            if($share_level){
                $levelname = $share_level->levelname;
            }
        }
        $res['data']['levelname'] = $levelname;
