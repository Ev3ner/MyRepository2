...
class OrderController extends Controller
{
    ...
    public function actionCancel($id = 0)
    {
        ...
        if (!$order) {
            return new \app\opening\ApiResponse(1, '订单不存在，或已取消');
        }
        ...
        if ($order->save()) {
            $t->commit();
            return new \app\opening\ApiResponse(0, '取消成功');
        } else {
            $t->rollBack();
            return new \app\opening\ApiResponse(1, '取消失败');
        }
    }
    ...
    public function actionPayData($id = 0)
    {
        ...
        return new \app\opening\BaseApiResponse($form->payData($id));
    }
    ...
    public function actionOrderDetails($id = 0)
    {
        ...
        return new \app\opening\ApiResponse(0, 'success', $order);
    }
    ...
    public function actionApplyRefund()
    {
        ...
        $order = YyOrder::find()
            ...
            ->one();
        if (!$order) {
            return new \app\opening\ApiResponse(1, '订单错误');
        }
        ...
        if ($order->save()) {
            ...
            $form->order_type = 3;
            $form->refund();
            return new \app\opening\ApiResponse(0, '退款申请成功');
        } else {
            return new \app\opening\ApiResponse(1, '退款申请失败,请重试');
        }
    }
